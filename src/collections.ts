...
export const collections: GMCollection[] = collection_data.collections.map((collection) => {
  const tokens = JSON.parse(fs.readFileSync(`./token_metadata/${collection.slug}.json`).toString());
  const attributes = JSON.parse(fs.readFileSync(`./token_metadata/${collection.slug}_attribute_dictionary.json`).toString());
  return {
    ...collection,
    tokens: tokens.root,
