...
const collections = collections_data.collections;
...
const dump_attribute_dictionary = (tokens: TokenData[][]) => {
  for (let collection of tokens) {
    ...
    fs.writeFileSync(`./token_metadata/${collections[collection[0].collectionId - 1].slug}_attribute_dictionary.json`, JSON.stringify(json, null, 2));
  }
}
